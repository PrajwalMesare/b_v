...
function nextStep() {
    if (currentStep === 1) currentStep = 3; // skip removed step
    else if (currentStep < totalSteps) currentStep++;
    showStep(currentStep);
    if (currentStep === 3) document.getElementById('envelope')?.classList.add('open');
}
...
function createPetals() {
    ...
    for(let i=0;i<15;i++){
        ...
        gsap.to(petal,{y:window.innerHeight+50,x:`+=${sway}`,rotation:360,duration:duration,delay:delay,ease:"none",onComplete:()=>{petal.remove();}});
    }
}
...
function typeMessage() {
    ...
    function type(){
        ...
        if(isDeleting){typingText.innerHTML=currentMessage.substring(0,charIndex-1);charIndex--;typingSpeed=50;}
        else{typingText.innerHTML=currentMessage.substring(0,charIndex+1);charIndex++;typingSpeed=100;}
        if(!isDeleting && charIndex===currentMessage.length){isDeleting=true;typingSpeed=1500;}
        else if(isDeleting && charIndex===0){isDeleting=false;messageIndex=(messageIndex+1)%messages.length;typingSpeed=500;}
...
function createFirework(){
    ...
    const firework=document.createElement('div');firework.classList.add('firework');
    ...
    setTimeout(()=>{for(let i=0;i<30;i++){const particle=document.createElement('div');particle.classList.add('firework-particle');particle.style.backgroundColor=colors[Math.floor(Math.random()*colors.length)];particle.style.left=firework.style.getPropertyValue('--x');particle.style.top=firework.style.getPropertyValue('--y');particle.style.setProperty('--tx',`${Math.cos(i*0.2)*100}px`);particle.style.setProperty('--ty',`${Math.sin(i*0.2)*100}px`);document.body.appendChild(particle);setTimeout(()=>particle.remove(),1000);}firework.remove();},1000);
}
...
function setCountdown(){
    ...
    function updateCountdown(){
        ...
        if(diff<=0){document.getElementById('countdown')?.innerHTML="<span>Happy Birthday!</span>";return;}
