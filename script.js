...
function showStep(step) {
    ...
    switch(step) {
        case 1: gsap.from("#envelope", { y: 100, opacity: 0, duration: 1, ease: "back.out(1.7)" }); break;
        case 2: gsap.from("#interactiveHeart", { scale: 0.5, rotation: 180, duration: 1, ease: "elastic.out(1, 0.5)" }); break;
        case 3: gsap.from(".photo-frame", { y: 50, rotation: -10, opacity: 0, duration: 1, ease: "back.out(1.7)" }); break;
        case 4: gsap.from(".polaroid", { y: 100, opacity: 0, stagger: 0.2, duration: 1, ease: "back.out(1.7)" }); break;
        case 5: createFireworks(); gsap.from(".heart", { scale: 0, duration: 1.5, ease: "elastic.out(1, 0.5)" }); break;
    }
}
...
function nextStep() {
    if (currentStep < totalSteps) {
        currentStep++;
        showStep(currentStep);
        if (currentStep === 2) document.getElementById('envelope').classList.add('open');
    }
}
...
function shareOnSocial(platform) {
    ...
    gsap.to(`.social-icon:nth-child(${['facebook','twitter','whatsapp'].indexOf(platform)+1})`, { scale:1.3, duration:0.3, yoyo:true, repeat:1 });
}
